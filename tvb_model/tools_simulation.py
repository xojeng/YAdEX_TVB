...
def init(parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
	...
	if parameter_connection_between_region['default']:
		...
		path = parameter_connection_between_region['path']
		connection = lab.connectivity.Connectivity.from_file(path)
	else:
	...
	if parameter_coupling['type'] == 'Linear':
		...
		coupling = lab.coupling.Sigmoidal(a=parameter_coupling['parameter']['a'],
		...
		coupling = lab.coupling.SigmoidalJansenRit(a=parameter_coupling['parameter']['a'],
                                       b=parameter_coupling['b'],
									   midpoint=parameter_coupling['parameter']['midpoint'],
		...
		coupling = lab.coupling.PreSigmoidal(H=parameter_coupling['parameter']['H'],
                                       b=parameter_coupling['b'],
...
def gen_bif_data(rangeG, parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
	...
	for G in rangeG:
		for initconds in ['low', 'high']:
			list_arg.append([G,initconds])

	def mp_sim(arg):
		return run_bif_data_one_G(arg[0]. arg[1], parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
		 parameter_integrator,parameter_monitor)

	pool = mp.ProcessingPool(process=4)
	pool.map(dill.copy(mp_sim), list_arg)
	return

def run_bif_data_one_G(G, initconds, parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
		 parameter_integrator,parameter_monitor):

	parameter_monitor['Raw']= True
	parameter_monitor['TemporalAverage']=False
	parameter_monitor['Bold']=False

	parameter_integrator['stochastic'] = False

	nregions = parameter_connection_between_region['number_of_regions']
	path = parameter_simulation['path_result']

	parameter_coupling['parameter']['a'] = G

	if initconds == 'low':
		initial_conditions = np.zeros((1, 7, nregions, 1))
		initial_conditions[0, 0] = np.random.uniform(low=0., high=0.001, size=(nregions, 1))
		initial_conditions[0, 1] = np.random.uniform(low=0., high=0.001, size=(nregions, 1))
		parameter_simulation['path_result']=path+"low_G_%s/"%str(G)
	elif initconds == 'high':
		initial_conditions = np.zeros((1, 7, nregions, 1))
		initial_conditions[0, 0] = np.random.uniform(low=0.01, high=0.02, size=(nregions, 1))
		initial_conditions[0, 1] = np.random.uniform(low=0.01, high=0.02, size=(nregions, 1))
		parameter_simulation['path_result']=path+"high_G_%s/"%str(G)

	simulator = init(parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
	parameter_integrator,parameter_monitor, initial_conditions)

	run_simulation(simulator, 5000, parameter_simulation,parameter_monitor)
